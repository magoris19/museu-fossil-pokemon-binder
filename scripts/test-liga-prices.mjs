...
assert.equal(evaluate('cards.filter(c => ligaPriceFor(c) != null).length'), 291);
...
assert.equal(evaluate("ligaPriceFor(getCard('sv03.5-138'))"), 2.99);
assert.equal(evaluate("ligaPriceFor(getCard('sv03.5-139'))"), 12);
assert.equal(evaluate("ligaPriceFor(getCard('sv03.5-140'))"), 7.67);
...
for (const direction of [1, -1]) {
  ...
  assert.ok(prices.slice(291).every(p => p === null));
  for (let i = 1; i < 291; i++) assert.ok(direction * (prices[i] - prices[i - 1]) >= 0);
}
...
console.log('OK: 303 cards, 291 references; identity, Holon correction, Liga official overrides, sorting and paid amount isolation.');
